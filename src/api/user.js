...
export async function loginUser(username, password) {
    let userDetails = await fetch('https://invoice-gst.herokuapp.com/users/login', {
        method: 'POST',
...
export async function registerUser({ username, password, name, email }) {
    let userDetails = await fetch('https://invoice-gst.herokuapp.com/users/register', {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({
            username,
...
export async function getUsernameAvailable(username) {
    let isAvailable = await fetch('https://invoice-gst.herokuapp.com/users?isUsernameAvail=' + username)
        .then(res => res.json())
    return isAvailable
}

export async function getSingleUserDetails(token) {
    let userDetails = await fetch('https://invoice-gst.herokuapp.com/users/' + token, {
        method: 'GET',
        headers: {
